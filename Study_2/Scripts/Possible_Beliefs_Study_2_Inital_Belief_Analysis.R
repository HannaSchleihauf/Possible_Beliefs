...
# Study 2
# Belief analysis with random slopes
# March 2021
...
# or for results load
# load("./Study_2/R_Images/comp_sep_belief_analysis_with_inital_model.RData")
...
for (i in 1:(length(to.change.1))) { # i = 1
  ...
  if (length(full1$warnings) == 0 &
    ...
    emm.pairs.full <-
      summary(
        contrast(emm, "pairwise")[c(
          1, 2, 3, 6, 9, 11, 14, 18,
          21, 22, 23, 24, 29, 33, 34, 35, 36
        )],
        type = "response",
        adjust = "sidak"
      )
...
save.image(
  "./Study_2/R_Images/comp_sep_belief_analysis_with_inital_model.RData"
)
# load("./Study_2/R_Images/comp_sep_belief_analysis_with_inital_model.RData")
...
round(mean(
  all.res$test.2.way.Condition.Age.3.Pr..Chisq[
...
round(mean(
  all.res$test.main.Age.3.Pr..Chisq[
...
round(mean(all.res$test.main.Condition.Pr..Chisq[
...
emm.post.hoc.full.cis$term <- paste(
  emm.post.hoc.full.cis$Condition,
  emm.post.hoc.full.cis$Age.3
)

mean.prob <- round(tapply(emm.post.hoc.full.cis$prob,
  emm.post.hoc.full.cis$term,
  FUN = mean
), 3)
min.prob <- round(tapply(emm.post.hoc.full.cis$prob,
  emm.post.hoc.full.cis$term,
  FUN = min
), 3)
max.prob <- round(tapply(emm.post.hoc.full.cis$prob,
  emm.post.hoc.full.cis$term,
  FUN = max
), 3)

min.CI <- round(tapply(emm.post.hoc.full.cis$asymp.LCL,
  emm.post.hoc.full.cis$term,
  FUN = min
), 3)
max.CI <- round(tapply(emm.post.hoc.full.cis$asymp.UCL,
  emm.post.hoc.full.cis$term,
  FUN = min
), 3)
mean.CI.lower <- round(tapply(emm.post.hoc.full.cis$asymp.LCL,
  emm.post.hoc.full.cis$term,
  FUN = mean
), 3)
mean.CI.upper <- round(tapply(emm.post.hoc.full.cis$asymp.UCL,
  emm.post.hoc.full.cis$term,
  FUN = mean
), 3)
cbind(min.CI, max.CI)
...
min.CI <- round(tapply(emm.post.hoc.full$asymp.LCL,
  emm.post.hoc.full$term,
  FUN = min
), 3)
max.CI <- round(tapply(emm.post.hoc.full$asymp.UCL,
  emm.post.hoc.full$term,
  FUN = min
), 3)
mean.CI.lower <- round(tapply(emm.post.hoc.full$asymp.LCL,
  emm.post.hoc.full$term,
  FUN = mean
), 3)
mean.CI.upper <- round(tapply(emm.post.hoc.full$asymp.UCL,
  emm.post.hoc.full$term,
  FUN = mean
), 3)
cbind(min.CI, max.CI)
...
xx <- data.frame(
  ...
  Condition = c(
    rep("Immoral", 3),
    rep("Moral", 3), rep("Opinion", 3)
  ),
  Age.3 = c(rep(c("5-6 group", "7-8 group", "Adult"), 3)),
  prob = mean.prob, asymp.LCL = mean.CI.lower,
...
p <- p + scale_x_discrete(labels = c("5-6 y/o", "7-8 y/o", "Adults")) +
  theme(
